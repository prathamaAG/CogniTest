...
$paper_query = "SELECT p.*, c.name AS course_name, c.code AS course_code
                FROM papers p
                JOIN courses c ON p.course_id = c.id
                WHERE p.access_key = ? AND p.faculty_id = ? AND p.status = 'approved'";
$paper_stmt = $conn->prepare($paper_query);
if (!$paper_stmt) {
    die("Prepare statement failed: " . $conn->error);
}
$paper_stmt->bind_param("si", $access_key, $user_id);
$paper_stmt->execute();
...
function generateHTMLForPDF($paper, $section_A, $section_B, $section_C) {
    ...
    <head>
        ...
        <style>
            body { font-family: "Times New Roman", Times, serif; margin: 40px; line-height: 1.6; font-size: 12pt; }
            .header { text-align: center; margin-bottom: 30px; }
            .university-name { font-size: 16pt; font-weight: bold; margin-bottom: 5px; text-transform: uppercase; }
            .institute-name { font-size: 14pt; font-weight: bold; margin-bottom: 5px; }
            .department-name { font-size: 13pt; margin-bottom: 15px; }
            .course-details { font-size: 13pt; font-weight: bold; margin-bottom: 20px; }
            .exam-info { display: flex; justify-content: space-between; margin-bottom: 20px; border-top: 1px solid #000; border-bottom: 1px solid #000; padding: 10px 0; }
            .left-info, .right-info { width: 48%; }
            .instructions { margin-bottom: 20px; border: 1px solid #ccc; padding: 10px 15px; background-color: #f9f9f9; }
            .instructions h3 { margin-top: 0; }
            .section-header { font-weight: bold; margin-top: 25px; margin-bottom: 15px; font-size: 14pt; border-bottom: 1px solid #999; padding-bottom: 5px; }
            .question { margin-bottom: 20px; }
            .question-header { margin-bottom: 5px; line-height: 1.4; }
            .best-luck { text-align: center; margin-top: 40px; font-weight: bold; font-style: italic; font-size: 14pt; }
        </style>
    </head>
    <body>
        <div class="header">
    ...
    $html .= '<div class="best-luck"><p>Best of Luck</p></div>
    </body>
    </html>';
    return $html;
}
...
<body>
    <div class="preview-controls">
        ...
        <div class="control-buttons">
            <a href="view_paper.php" class="btn btn-outline-light btn-sm">
